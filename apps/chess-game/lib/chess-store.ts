...
export const useChessStore = create<ChessStore>((set, get) => ({
  ...
  removePlayer: (id: string) => {
    ...
    set({
      players: newPlayers,
      currentPlayer: currentPlayer?.id === id ? newPlayers[0] || null : currentPlayer,
    })
  },
  ...
  assignColors: () => {
    ...
    const colors: ("white" | "black")[] = Math.random() > 0.5 ? ["white", "black"] : ["black", "white"]
      ...
      gameState: {
        ...
        capturedPieces: capturedPiece
          ? {
              ...
              [capturedPiece.color]: [...state.gameState.capturedPieces[capturedPiece.color], capturedPiece],
            }
          : state.gameState.capturedPieces,
      },
    }))
  ...
  leaveTable: () => {
    ...
    const updatedPlayers = currentTable.players.filter((p) => p.id !== currentPlayer.id)
    const updatedTable = {
      ...currentTable,
    ...
    set((state) => ({
      tables: state.tables.map((t) => (t.id === currentTable.id ? updatedTable : t)),
      currentTable: null,
      currentPlayer: null,
      players: [],
    }))
